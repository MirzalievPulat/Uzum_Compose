package uz.gita.domain.transferUseCase.impl

import kotlinx.coroutines.flow.Flow
import uz.gita.common.data.TransferData
import uz.gita.domain.transferUseCase.GetCardOwnerUC
import uz.gita.domain.transferUseCase.GetFeeUC
import uz.gita.domain.transferUseCase.GetPhoneNumberUC
import uz.gita.entity.repository.TransferRepository
import javax.inject.Inject

class GetPhoneNumberUCImpl @Inject constructor(private val transferRepository: TransferRepository):GetPhoneNumberUC {
    override fun invoke(): Flow<Result<String>> {
        return transferRepository.getPhoneNumber()
    }
}